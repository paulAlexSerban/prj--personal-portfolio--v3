...
function handleRelearning(card: CardState, rating: Rating, config: StudyConfig, now: number) {
...
function scheduleLearning(card: CardState, minutes: number, now: number) {
    card.learningDueAt = now + Math.round(minutes * 60_000);
    card.dueDate = todayISO(0);
}

function graduate(card: CardState, config: StudyConfig, easy: boolean) {
    card.cardType = "review";
...
function handleReview(card: CardState, rating: Rating, config: StudyConfig, _now: number) {
