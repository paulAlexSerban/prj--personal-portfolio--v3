...
of them. Stage uses **live** content (private content-repo sync) and is gated by
HTTP Basic Auth, same as test.

```
stage.paulserban.eu           -> CloudFront (Basic Auth) -> S3 (portfolio)
stage.blog.paulserban.eu      -> CloudFront (Basic Auth) -> S3 (blog)
stage.quiz.paulserban.eu      -> CloudFront (Basic Auth) -> S3 (quiz SPA)
stage.news-feed.paulserban.eu -> CloudFront (Basic Auth) -> S3 (news)
...
module name `static_site` for consistency with `envs/test`).
...
- Before the first apply, confirm the `stage.*` CNAMEs are not already attached
...
AWS account. The **test** env creates it (`create_oidc_provider = true`). This stage
...
role. It does **not** delete the shared OIDC provider (stage did not create it).
