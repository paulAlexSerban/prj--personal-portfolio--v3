...
Before deciding, several architectural alternatives were considered, each with distinct strengths and weaknesses for a portfolio project.
...
| **Pros**                                                                               | **Cons**                                                                                                                    |
| -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Smooth client-side navigation with no page reloads, leading to better user experience. | SEO: SPAs can have SEO challenges since search engines might struggle to index dynamic content rendered on the client side. |
| API calls can update content dynamically on the client side.                           | Performance: Large SPAs can suffer from performance issues due to the need to load all JavaScript upfront.                  |
...
| **Pros**                                                                                                                          | **Cons**                                                                                                                                      |
| --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Offers full control over server-side logic and database interactions, allowing complex business logic and highly dynamic content. | Performance: Slower load times due to repeated server round trips and heavier backend infrastructure.                                         |
| Easy integration with traditional server-side frameworks (e.g., Django, Rails, Laravel).                                          | Scalability: Requires more effort to scale, both in terms of servers and databases, as each page load requires a full request-response cycle. |
...
- **Integration with APIs**: As the project grows, additional features can be added by integrating third-party APIs or backend microservices, maintaining the flexibility to scale without affecting the frontend.
